...
var SEVERITY = { Extreme: 100, Severe: 80, Moderate: 55, Minor: 30, Unknown: 15 };
var URGENCY = { Immediate: 100, Expected: 65, Future: 35, Past: 10, Unknown: 15 };
var CERTAINTY = { Observed: 100, Likely: 70, Possible: 40, Unlikely: 15, Unknown: 15 };
var scoreAlert = (severity, urgency, certainty) => Math.round(0.5 * (SEVERITY[severity] ?? 15) + 0.3 * (URGENCY[urgency] ?? 15) + 0.2 * (CERTAINTY[certainty] ?? 15));
var fetchAndScore = (sendRequester, config) => {
  ...
  let best = { riskScore: 0, alertId: "", event: "none" };
  for (const f of data.features ?? []) {
    const p = f.properties;
    if (!/flood/i.test(p.event ?? ""))
      continue;
    const s = scoreAlert(p.severity, p.urgency, p.certainty);
    if (s > best.riskScore)
      best = { riskScore: s, alertId: p.id ?? "", event: p.event };
  }
  return best;
};
var doHazard = (runtime2) => {
  runtime2.log(`Fetching hazard data: ${runtime2.config.url}`);
  const http2 = new cre.capabilities.HTTPClient;
  const result = http2.sendRequest(runtime2, fetchAndScore, ConsensusAggregationByFields({
    riskScore: median,
    alertId: identical,
    event: identical
  }))(runtime2.config).result();
  runtime2.log(`Top flood risk: event="${result.event}" riskScore=${result.riskScore} alertId=${result.alertId}`);
  const unlocked = result.riskScore >= runtime2.config.riskThreshold;
  runtime2.log(unlocked ? `riskScore ${result.riskScore} >= threshold ${runtime2.config.riskThreshold}: RELEASE UNLOCKED` : `riskScore ${result.riskScore} < threshold ${runtime2.config.riskThreshold}: no release`);
  return JSON.stringify(result);
};
var onCronTrigger = (runtime2, _payload) => {
  runtime2.log("Running CivicShield hazard scoring workflow");
  return doHazard(runtime2);
};
